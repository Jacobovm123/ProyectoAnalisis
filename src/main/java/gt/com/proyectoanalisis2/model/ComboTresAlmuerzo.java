...
public class ComboTresAlmuerzo implements InterfazOrderControl{
    @Override
    public Campos_pedido createOrder(HttpServletRequest request) {
        ...
        alternativa.add(request.getParameter("alt_lomito"));
        alternativa.add(request.getParameter("alt_chorizo"));
        ...
        nuevoMenu.add(menu.menus(request.getParameter("combo")).get(4)+" ["+alternativa.get(4)+"]");
